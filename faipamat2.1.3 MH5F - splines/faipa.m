...
function [x,f,g,lambda0,mu0,counter,x_iter] = ...
...
...
% nbind - NUMBER OF INEQUALITY BINDING CONSTRAINTS
x=x(:);

check_input(x,vlb,vub,nvar,ncstr,neq,neqlin,lvlb,lvub)
...
% INITIAL PENALTY PARAMETER c
      ...
      g=zeros(0,1);bind=zeros(0,1);nbind=0;
     ...
     if ifpoint==0 %%>>> 16/10 
         ...
         if ifilt == 0
            ...
            compute(x,g,fun,gfun,neq,neq_op,ncstr,icall,ilsearch,ifilt,bind,nbind,...
                    counter, nprob,data,idata,iutil,rutil); %gradg,pause
                 
                 
         else
                 
            icall=12; %%>>> 16/10
            [f,g_all,bind,nbind,gradfdum,gradgdum,counter]=...
            ...
            compute(x,g,fun,gfun,neq,neq_op,ncstr,icall,ilsearch,ifilt,bind,nbind,...
                    counter, nprob,data,idata,iutil,rutil); %gradg,pause
                 
            [bind,nbind]=filtcons(g_all,neq,ncstr,iutil,rutil);

            icall=13; %%>>> 16/10
            [fdum,g_alldum,bind,nbind,gradf,gradg,counter]=...
            ...
            compute(x,g,fun,gfun,neq,neq_op,ncstr,icall,ilsearch,ifilt,bind,nbind,...
                    counter, nprob,data,idata,iutil,rutil); %gradg,pause
         ...
         if istop==0
            ...
            compute(x,g,fun,gfun,neq,neq_op,ncstr,icall,ilsearch,ifilt,...
                    bind,nbind,counter, nprob,data,idata,iutil,rutil); %gradg,pause
         end 
         ...
         if ifilt==1
            if ifpoint == 0
               ...
               [bind,nbind]=filtconsFP(x(nvar),g_all,ncstr,iutil,rutil);
            end 
         end   
...
% UPDATING QUASI-NEWTON MATRIX B 
         ...
         if iupdateb==1 & initial==0
            if isolver==1 | isolver==4 | iredbox==1
               ...
               [B,iterB]=updatb(iterB,B,x,oldx,auxvec,gradf, ...
                ...
                [B,iterB]=updath(iterB,B,x,oldx,t,auxvec,gradf, ...
                   gradg,glag,rho2,glag2,bind,bindold,mu0,lambda0,...
                                modg,nvar,ncstr,neq);
         ...
         if iupdateb==2 & initial==0
            ...
            [DDELTA,GGAMA,DMAT,RMAT,GAMAtGAMA,npairs]=...
               updath_lim(DDELTA,GGAMA,DMAT,RMAT,GAMAtGAMA,x,oldx,t,auxvec,gradf, ...
               gradg,glag,rho2,glag2,bind,bindold,mu0,...
...
%            CALL 2
            ...
            [f,g_all,bind,nbind,gradf,gradg(:,1:neq),counter]=...
            ...
...
% CALL 5
     ...
     %          bind=[]; nbind=0;
               ...
               [auxvec]=update_aux(auxvec,gradg,bind,bindold,...
                                   lambda0,ncstr,neq);
            ...
            if iterlin >= idata(9)
               disp('ERROR, The number of iterations in line search is maximum')
               istop=5;
            else          
               iterlin=iterlin + 1;
            end 
